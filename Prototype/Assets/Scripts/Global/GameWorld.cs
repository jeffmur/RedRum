...
public partial class GameWorld : SceneSingleton<GameWorld>
{
    ...
    private void FixedUpdate()
    {
        ...
        if(mCamera != null)
        {
            ...
            Vector3 desiredPosition = (crosshairs.transform.position + casper.transform.position) / 2;
            float dist = Vector2.Distance(mCamera.transform.position, casper.transform.position);
            // Always keep Casper in view
            if (dist >= 10)  desiredPosition = casper.transform.position;
            // Smooth Lerp to Pos
